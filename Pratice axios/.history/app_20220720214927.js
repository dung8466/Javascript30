...
function displayMatch(){
    ...
    const valueReturn = city.map(value =>{
        ...
        const cityName = value.city.replace(regex, (match) => `<span class="hl">${value}</span>`);
        const provinceName = value.province.replace(regex, (match) => `<span class="hl">${value}</span>`);
        // return `
        //     <li>
        //         <span class='name'>${value.city}, ${value.province}</span>
